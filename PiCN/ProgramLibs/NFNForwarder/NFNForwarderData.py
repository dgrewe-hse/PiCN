"""NFN Forwarder for PICN (data-offloading chunk layer variant).

Async runtime for this class is deferred: ``DataOffloadingChunklayer`` has
no asyncio wrapper yet (Phase 5.5). Pass ``runtime=async`` to get a clear
``NotImplementedError``. Sync path is unchanged.
"""
...
class NFNForwarderData(object):
    """NFN Forwarder for PICN with DataOffloadingChunklayer."""
    ...
    def __init__(
        ...
        if runtime is Runtime.ASYNC:
            raise NotImplementedError(
                "NFNForwarderData async runtime is deferred until "
                "DataOffloadingChunklayer has an AsyncLayerProcess wrapper "
                "(see docs/baseline.md After Phase 5)."
            )
        ...
        self.chunklayer = DataOffloadingChunklayer(
            cs, pit, fib, num_of_forwards=num_of_forwards,
            chunk_size=chunk_size, log_level=log_level)
        ...
        self.mgmt = Mgmt(
            self.icnlayer.cs, self.icnlayer.fib, self.icnlayer.pit,
            self.linklayer, mgmt_port, self.stop_forwarder, log_level=log_level)
